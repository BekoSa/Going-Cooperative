...
foreach ($gate in @("beamPlacementReplication", "beamLifecycleReplication", "socketablePlacementReplication")) {
    Require-SourcePattern $sources.Config ('(?m)^\s*' + $gate + '\s*=\s*true\s*(?:[#;].*)?Require-SourcePattern $sources.CorePolicy 'CurrentTransactionSchemaVersion\s*=\s*2\s*;' `
...
Require-SourcePattern $sources.Config '(?m)^\s*beamReplicationDiagnostics\s*=\s*false\s*(?:[#;].*)?Require-SourcePattern $sources.CorePolicy 'CurrentTransactionSchemaVersion\s*=\s*2\s*;' `
    "Tagged beam/socket placement did not bump the strict Building V2 schema."
