...
if (-not $captureSource.Contains('ReplicationPendingHostBuildReplayChunks') -or -not $captureSource.Contains('ProcessPendingReplicationHostBuildReplayChunks') -or -not $captureSource.Contains('if (durablePending > 0)')) { throw "Host-origin replay chunks are not staged and ACK-paced before reliable registration." }
