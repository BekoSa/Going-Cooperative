...
if (-not $captureSource.Contains('ReplicationClientBuildCommandChunkPlacements = 4') -or -not $captureSource.Contains('holds every later BuildBatch until the previous')) { throw "Client-origin large drags are not serialized into bounded sequential host-apply chunks." }
if (-not $captureSource.Contains('ReplicationHostBuildReplayChunkPlacements = 4')) { throw "Host-origin replay is not bounded to small client-apply chunks." }
if (-not $captureSource.Contains('ReplicationPendingHostBuildReplayChunks') -or -not $captureSource.Contains('ProcessPendingReplicationHostBuildReplayChunks') -or -not $captureSource.Contains('ReplicationHostBuildReplayMaxInFlightChunks = 4') -or -not $captureSource.Contains('if (durablePending >= ReplicationHostBuildReplayMaxInFlightChunks)')) { throw "Host-origin replay chunks are not staged behind the bounded reliable in-flight window." }
if (-not $runtimeSource.Contains('ProcessPendingReplicationHostBuildReplayChunks();') -or -not $runtimeSource.Contains('ClearPendingReplicationHostBuildReplayChunks();')) { throw "Host runtime does not pump/clear staged host build replay around resync." }
